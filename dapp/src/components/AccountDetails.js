...
function AccountDetails({ children }) {
    const { address, balance, getUserInfo, updateConnectedChain, connectedChain, isLoading, chainList, switchChain, getChainId, chainListOptionSelected, } = usePlayground();
    ...
    return (React.createElement("div", { className: "py-16 w-11/12 px-4 sm:px-6 lg:px-8 z-0" },
        ...
        React.createElement("div", { className: "p-8 mt-6 mb-0 rounded-lg bg-white flex flex-row justify-between flex-wrap" },
            ...
            React.createElement("div", { className: "p-2 flex flex-col space-y-4" },
                ...
                React.createElement("span", { className: "text-2xl font-bold" }, connectedChain?.chainId || ""))),
        React.createElement("div", { className: "p-8 mt-6 rounded-lg bg-white flex flex-col space-y-4" },
            React.createElement("h2", { className: "text-lg font-bold" }, "Use Custom Chain Config"),
            React.createElement("form", { onSubmit: handleSubmit, className: "space-y-4" },
                Object.entries(chainDetails).map(([field, value], index) => field !== "chainNamespace" && (React.createElement("div", { key: index, className: "flex items-center space-x-2" },
                    React.createElement("label", { htmlFor: field, className: "text-sm min-w-[120px]" },
                        field[0].toUpperCase() + field.slice(1).replace(/([A-Z])/g, " $1"),
                        ":"),
                    React.createElement("input", { type: "text", id: field, value: value || "", onChange: (e) => setChainDetails({ ...chainDetails, [field]: e.target.value }), className: "form-input flex-1" }),
                    " "))),
                isLoading ? (React.createElement("svg", { className: "animate-spin -ml-1 mr-3 h-5 w-5 text-white", xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24" },
                    ...
                    React.createElement("path", { className: "opacity-75", fill: "#0364ff", d: "M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" }))) : (React.createElement("button", { type: "submit", className: "flex flex-row rounded-full px-6 py-3 text-white justify-center items-center cursor-pointer", style: { backgroundColor: "#0364ff" }, onClick: () => switchChain(chainDetails) }, "Change Network Config")))),
        children));
}
